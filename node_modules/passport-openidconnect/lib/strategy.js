...
function Strategy(options, verify) {
  ...
  this._skipUserInfo = (options.skipUserInfo === undefined) ? false : options.skipUserInfo;
  ...
  if (req.query && req.query.code) {
    ...
    this.configure(identifier, function(err, config) {
      ...
      oauth2.getOAuthAccessToken(code, { grant_type: 'authorization_code', redirect_uri: callbackURL }, function(err, accessToken, refreshToken, params) {
        ...
        self._shouldLoadUserInfo(iss, sub, function(err, load) {
          if (err) { return self.error(err); };
          ...
          if (load) {
            ...
            oauth2._request("GET", userInfoURL, { 'Authorization': "Bearer " + accessToken, 'Accept': "application/json" }, null, null, function (err, body, res) {
              if (err) { return self.error(new InternalOAuthError('failed to fetch user info', err)); }

              // TODO: verify the sub Claim in the UserInfo response...
          ...
          function onUserInfoLoaded(userInfo) {
            ...
            if (self._passReqToCallback) {
              ...
              if (arity == 9) {
                self._verify(req, iss, sub, userInfo, jwtClaims, accessToken, refreshToken, params, verified);
              } else if (arity == 8) {
                self._verify(req, iss, sub, userInfo, accessToken, refreshToken, params, verified);
              } else if (arity == 7) {
                self._verify(req, iss, sub, userInfo, accessToken, refreshToken, verified);
              } else if (arity == 5) {
                self._verify(req, iss, sub, userInfo, verified);
              } else { // arity == 4
              ...
              var arity = self._verify.length;
              if (arity == 8) {
                self._verify(iss, sub, userInfo, jwtClaims, accessToken, refreshToken, params, verified);
              } else if (arity == 7) {
                self._verify(iss, sub, userInfo, accessToken, refreshToken, params, verified);
              } else if (arity == 6) {
                self._verify(iss, sub, userInfo, accessToken, refreshToken, verified);
              } else if (arity == 4) {
                self._verify(iss, sub, userInfo, verified);
              } else { // arity == 3
...
/**
 * Check if should load user userInfo, contingent upon options.
 *
 * @param {String} issuer
 ...
 * @api private
 */
Strategy.prototype._shouldLoadUserInfo = function(issuer, subject, done) {
  if (typeof this._skipUserInfo == 'function' && this._skipUserInfo.length > 1) {
    // async
    this._skipUserInfo(issuer, subject, function(err, skip) {
      if (err) { return done(err); }
      if (!skip) { return done(null, true); }
      return done(null, false);
    });
  } else {
    var skip = (typeof this._skipUserInfo == 'function') ? this._skipUserInfo(issuer, subject) : this._skipUserInfo;
    if (!skip) { return done(null, true); }
    return done(null, false);
  }
}
